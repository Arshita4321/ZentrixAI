...
router.get('/reports/:id/pdf', async (req, res) => {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="ZentrixAI_Memo_${report.resolvedTicker || 'Report'}.pdf"`
    );
    res.send(pdfBuffer);
  } catch (err) {
  ...
  try {
    ...
    const response = await withTimeout(
      model.invoke([
        [
          'system',
          'You are Zentrix AI Institutional AI Assistant. Answer the user follow-up question strictly grounded in the research report context provided below. Do NOT invent or hallucinate facts outside the provided context. If context does not contain the answer, state clearly what data is available in the report.'
        ],
        [
          'user',
...
router.post('/reports/:id/chat', requireAuth, async (req, res) => {
  try {
    ...
    const systemPrompt = [
      `You are Zentrix AI's research assistant for ${companyName}. Answer the user's question using the research data provided above as your primary source.`,
