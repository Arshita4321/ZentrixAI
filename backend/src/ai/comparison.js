...
export async function runComparisonNode(reportA, reportB) {
  ...
  const prompt = `
You are ZentrixAI Institutional Allocation Committee. Perform a direct head-to-head comparison between two researched companies:
- COMPANY A: ${nameA}
- COMPANY B: ${nameB}
...
`.trim();
  ...
  const systemPrompt = `You are ZentrixAI AI Allocation Committee synthesizing a head-to-head comparative investment memo. Output valid JSON matching the schema.`;
