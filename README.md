# ZentrixAI — Institutional AI Equity Research Terminal & Decision Engine

[![Live Frontend Demo](https://img.shields.io/badge/Live%20Demo-ZentrixAI%20Frontend-00D8B6?style=for-the-badge&logo=render&logoColor=white)](https://ZentrixAI-frontend-ze47.onrender.com)
[![Live Backend API](https://img.shields.io/badge/API%20Server-ZentrixAI%20Backend-4F46E5?style=for-the-badge&logo=render&logoColor=white)](https://ZentrixAI-backend-0yw5.onrender.com/api/health)

**ZentrixAI** is an autonomous, enterprise-grade AI investment research terminal built on **MERN (MongoDB, Express, React, Node.js)** and **LangGraph.js**. It orchestrates a committee of specialized autonomous AI agents to investigate companies from multiple angles, conduct an adversarial **Bull vs. Bear debate**, and compute an explainable `INVEST / WATCH / PASS` verdict with granular confidence scoring, head-to-head comparisons, and interactive grounded Q&A.
...
| :--- | :--- | :--- |
| **Frontend Terminal** | [https://ZentrixAI-frontend-ze47.onrender.com](https://ZentrixAI-frontend-ze47.onrender.com) | Public marketing portal & institutional research command center |
| **Backend API Service** | [https://ZentrixAI-backend-0yw5.onrender.com](https://ZentrixAI-backend-0yw5.onrender.com) | LangGraph.js SSE streaming engine & JWT auth service |
| **API Health Status** | [https://ZentrixAI-backend-0yw5.onrender.com/api/health](https://ZentrixAI-backend-0yw5.onrender.com/api/health) | Live diagnostics & LLM provider status |
...
- **Interactive "Ask ZentrixAI" Chatbot (`POST /api/reports/:id/chat`)**: Multi-turn conversational Q&A drawer grounded strictly in the completed report context.
- **Full JWT Authentication & Security Hardening (`/api/auth/*`)**: Hashed passwords (`bcryptjs`), secure httpOnly cross-domain JWT cookies (`ZentrixAI_token`), API rate limiting (`express-rate-limit`), security headers (`helmet`), and startup environment checks.
...
MONGODB_URI=mongodb://localhost:27017/ZentrixAI
...
ZentrixAI uses `@langchain/langgraph` to run a deterministic 8-node stateful graph (`backend/src/ai/graph.js`):
...
- **Confidence-Over-Time Longitudinal Charting (Deliberate Scope Decision)**: I excluded multi-month confidence trend plotting because the core value proposition of ZentrixAI is deep, adversarial point-in-time debate analysis rather than standard portfolio ticker tracking.
...
├── models/
│   ├── Chat.js              # Mongoose schema for Ask ZentrixAI chats
...
frontend/src/
├── components/
│   ├── AskZentrixAI.jsx          # Interactive Q&A chat modal/drawer
│   ├── AskZentrixAIChat.jsx      # Conversational UI component
