...
export const AskZentrixAI = ({
  ...
  const handleSendMessage = async (textToSend) => {
    ...
    try {
      ...
      setError(err.message || 'Connection failed while communicating with Zentrix AI.');
    } finally {
                ...
                <div className="p-4 rounded-2xl text-sm max-w-[85%] bg-cyan-950/30 border border-cyan-500/40 text-slate-100 rounded-bl-none">
                  {streamingContent ? (
                    ...
                    <div className="flex items-center gap-2 text-cyan-400 text-xs font-mono">
                      ...
                      <span>Zentrix AI synthesizing grounded response...</span>
                    </div>
                  )}
                </div>
              </div>
